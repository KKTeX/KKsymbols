...
\begin{description}
  ...
  \KKverb|autolowercase=1|. If the fully expanded argument consists only of
  lowercase ASCII letters (\texttt{a--z}), a non-starred enclosure command
  automatically uses the same sizing path as its starred form. This also
  recognizes lowercase Roman numerals and letters produced by \texttt{KKran},
  such as \KKverb|\Rrnum{8}| (which expands to \texttt{viii}) and
  \KKverb|\alphabetarray{2}| (which expands to \texttt{b}). Digits, uppercase
  letters, Japanese text, and mixed strings keep the normal sizing path.
  ...
  both \KKverb|\jegg{...}| and \KKverb|\jegg*{...}|. Set
  \KKverb|autolowercase=0| to disable both automatic behaviors and use manual
  star selection.
  ...
  stars on \KKverb|\hishi|, \KKverb|\kurohishi|, \KKverb|\maruhishi|, and
  \KKverb|\kuromaruhishi| now select an aspect-ratio-preserving lowercase
  path which constrains both glyph width and height to the diamond's inner
  square. A separate
  ...
  また、菱形4コマンドの明示スターは、字形の縦横比を保ちつつ幅と高さの両方を
  菱形の内接正方形へ収める小文字向け経路を選ぶようになりました。
...
With the default \KKverb|autolowercase=1| setting, fully expanded lowercase
ASCII arguments automatically use the lowercase-safe sizing path, so
...
With \KKverb|autolowercase=1| (the default), lowercase ASCII arguments are
...
\begin{itemize}
  \item \textbf{Unreleased} --- Added automatic lowercase sizing, configurable frame line widths, and the compatibility fixes described above. Version and release date are intentionally left undecided pending release confirmation.
