...
\date{Version 2.0.0a (2025/12/31)}
\begin{document}
...
\textbf{Naturally, these commands also work correctly in vertical writing environments. But only when they include math-mode (in vertical mode), sometimes they don't.} Also when you want to typeset \maru{あ}, for instance, in vertical mode, you should use \verb|\RotTate| command. 

\begin{SourceCode}{Input}{TeX}
  % In vertical mode
  \RotTate[0]\kakko{あ}\kakko{い}\kakko{う}\par
  \RotTate[90]\kakko{1}\kakko{1}\kakko{3}
\end{SourceCode}

\begin{OutPut}{Output}
  \parbox<t>{5\zw}{%
    \RotTate[0]\kakko{あ}\kakko{い}\kakko{う}\par
    \RotTate[90]\kakko{1}\kakko{2}\kakko{3}
  }
\end{OutPut}
...
\begin{description}
  ...
  \item[In vertical mode] The default value is \verb|90|. 
\end{description}
...
\begin{itemize}
  ...
  \item \textbf{v2.0.0a (2025/12/31)} --- Added explanations regarding several important notes.
\end{itemize}
...
\begin{lstlisting}
  ...
  \ProvidesPackage{KKsymbols}[2025/12/31, Version 2.0.0a]
